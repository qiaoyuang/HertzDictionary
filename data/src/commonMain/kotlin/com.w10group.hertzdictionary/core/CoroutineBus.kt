...
object CoroutineBus {
    ...
    private val map = HashMap<KClass<*>, ArrayList<out Pair<OnWorkListener<*>, Channel<*>>>>()

    private fun <T : Any> put(key: KClass<T>, value: ArrayList<Pair<OnWorkListener<T>, Channel<T>>>) {
        map[key] = value
    }

    @Suppress("UNCHECKED_CAST")
    private fun <T : Any> get(key: KClass<T>) = map[key] as ArrayList<Pair<OnWorkListener<T>, Channel<T>>>?

    fun <T : Any> register(clazz: KClass<T>, observer: OnWorkListener<T>, capacity: Int = 0) {
        get(clazz) ?: ArrayList<Pair<OnWorkListener<T>, Channel<T>>>().also { put(clazz, it) }
